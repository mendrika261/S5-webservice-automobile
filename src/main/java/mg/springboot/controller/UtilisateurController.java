...
public class UtilisateurController {
  ...
  public Response<?> connexion(String email, String motDePasse) {
    Optional<Utilisateur> utilisateur =
        utilisateurService.findByEmailAndMotDePasse(email, motDePasse);
    if (utilisateur.isPresent())
      return Response.send(HttpStatus.OK, "success",
                           tokenService.createFor(utilisateur.get()));
    return Response.send(HttpStatus.OK, "error",
  ...
  public Response<?> deconnexion(HttpServletRequest request) {
    ...
    // Response.denied("");
    return Response.send(HttpStatus.OK, "success", "Déconnexion réussie",
                         token);
  }
  ...
  public Response<?> modifyUtilisateur(@PathVariable String id,
